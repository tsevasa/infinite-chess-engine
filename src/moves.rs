...
impl SpatialIndices {
    ...
    pub fn remove(&mut self, x: i64, y: i64) {
        if let Some(v) = self.rows.get_mut(&y) {
            ...
            if v.is_empty() { self.rows.remove(&y); }
        }
        if let Some(v) = self.cols.get_mut(&x) {
            Self::remove_sorted(v, y);
            if v.is_empty() { self.cols.remove(&x); }
        }

        let d1 = x - y;
        if let Some(v) = self.diag1.get_mut(&d1) {
            Self::remove_sorted(v, x);
            if v.is_empty() { self.diag1.remove(&d1); }
        }
        let d2 = x + y;
        if let Some(v) = self.diag2.get_mut(&d2) {
            Self::remove_sorted(v, x);
            if v.is_empty() { self.diag2.remove(&d2); }
        }
    }
}
...
pub fn get_legal_moves(
    ...
    get_legal_moves_into(board, turn, special_rights, en_passant, game_rules, indices, &mut moves);
    moves
}
...
pub fn get_quiescence_captures(
    ...
    for ((x, y), piece) in &board.pieces {
        ...
        match piece.piece_type {
            ...
            PieceType::Rook => {
                generate_sliding_capture_moves(board, &from, piece, &[(1, 0), (0, 1)], Some(indices), out);
            }
            PieceType::Bishop => {
                generate_sliding_capture_moves(board, &from, piece, &[(1, 1), (1, -1)], Some(indices), out);
            }
            ...
            PieceType::Knightrider => {
                generate_sliding_capture_moves(board, &from, piece, &[(1, 2), (1, -2), (2, 1), (2, -1)], None, out);
            }
...
fn generate_pawn_moves(board: &Board, from: &Coordinate, piece: &Piece, special_rights: &HashSet<Coordinate>, en_passant: &Option<EnPassantState>, game_rules: &GameRules) -> Vec<Move> {
    ...
    let default_promos = [PieceType::Queen, PieceType::Rook, PieceType::Bishop, PieceType::Knight];
    let promotion_pieces: &[PieceType] = game_rules.promotion_types
        .as_ref()
        .map(|v| v.as_slice())
        ...
        if let Some(target) = board.get_piece(&capture_x, &capture_y) {
            ...
            if let Some(ep) = en_passant {
                if ep.square.x == capture_x && ep.square.y == capture_y {
                    moves.push(Move::new(*from, Coordinate::new(capture_x, capture_y), *piece));
                }
            }
        }
    }
...
fn generate_pawn_capture_moves(
    ...
    for dx in [-1i64, 1] {
        ...
        if let Some(target) = board.get_piece(&capture_x, &capture_y) {
            ...
            if ep.square.x == capture_x && ep.square.y == capture_y {
                out.push(Move::new(*from, Coordinate::new(capture_x, capture_y), *piece));
            }
        }
    }
}
    ...
    for (dx_raw, dy_raw) in directions {
        for sign in [1i64, -1i64] {
            ...
            loop {
                ...
                step += 1;
                if step > 50 { break; } // Safety for infinite board
            }
        }
    }
}
...
fn extend_captures_only(board: &Board, our_color: PlayerColor, moves_in: Vec<Move>, out: &mut Vec<Move>) {
    for m in moves_in {
...
fn ray_border_distance(from: &Coordinate, dir_x: i64, dir_y: i64) -> Option<i64> {
    ...
    unsafe {
        ...
        if dir_x == 0 {
            let raw = if dir_y > 0 { max_y - from.y } else { from.y - min_y };
            let limit = raw.min(MAX_INF_DISTANCE);
            if limit > 0 { Some(limit) } else { None }
        } else if dir_y == 0 {
            let raw = if dir_x > 0 { max_x - from.x } else { from.x - min_x };
            let limit = raw.min(MAX_INF_DISTANCE);
            if limit > 0 { Some(limit) } else { None }
        } else if dir_x.abs() == dir_y.abs() {
            let raw_x = if dir_x > 0 { max_x - from.x } else { from.x - min_x };
            let raw_y = if dir_y > 0 { max_y - from.y } else { from.y - min_y };
        ...
        for sign in [1i64, -1i64] {
            ...
            for &(px, py, is_enemy) in &pieces_data {
                ...
                let on_ray = if is_vertical {
                    ...
                    dx.abs() == dy.abs() && dx != 0 && dx.signum() == dir_x.signum() && dy.signum() == dir_y.signum()
                };
            ...
            let (max_dist, border_dist) = if closest_dist < i64::MAX {
                let md = if closest_is_enemy { closest_dist } else { closest_dist - 1 };
                (md, None)
            } else {
            ...
            if closest_dist < i64::MAX {
                let wr = if closest_is_enemy { ENEMY_WIGGLE } else { FRIEND_WIGGLE };
            ...
            for d in target_dists {
                ...
                if !in_bounds(sq_x, sq_y) { continue; }
                if d == closest_dist && !closest_is_enemy { continue; }
